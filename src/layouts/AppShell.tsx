...
const MENUS: Record<Role, NavItem[]> = {
  consumer: [
    { label: 'Browse offers', to: '/',         icon: '🎁' },
    { label: 'My claims',     to: '/claims',   icon: '📄' },
    { label: 'Profile',       to: '/profile',  icon: '👤' },
  ],
  merchant: [
    { label: 'Dashboard',       to: '/partner',           icon: '📊' },
    { label: 'My offers',       to: '/partner/offers',    icon: '🏷️' },
    { label: 'Incoming claims', to: '/partner/claims',    icon: '📥' },
    { label: 'Profile',         to: '/profile',           icon: '👤' },
  ],
  admin: [
    { label: 'Overview',  to: '/admin',         icon: '📈' },
    { label: 'Users',     to: '/admin/users',   icon: '👥' },
    { label: 'Offers',    to: '/admin/offers',  icon: '🏷️' },
  ],
};
    ...
    <div className="min-h-screen flex flex-col md:flex-row">
      ...
      <aside className="md:w-60 md:min-h-screen bg-ink-900 text-white md:flex md:flex-col md:flex-shrink-0">
        <div className="px-5 py-5 border-b border-white/10 flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-brand flex items-center justify-center text-lg">⏱️</div>
          <div>
      ...
      <main className="flex-1 min-w-0">
        <div className="md:hidden flex items-center justify-between px-4 py-3 bg-white border-b border-ink-100">
          ...
          <button onClick={() => { logout(); navigate('/login', { replace: true }); }}
            className="text-sm text-brand-600 font-medium">Sign out</button>
        </div>
